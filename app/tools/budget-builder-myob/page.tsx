...
async function generateExcel(
  ...
  for (let j = 1; j <= 3; j++) {
    finRows.push(addPLRow([null, `Other cash item ${j}`, ...zeros28()]))
  }
  ...
  addBlank()
  const itHdrRn = wsPL.addRow([`INCOME TAX — For Information Only (not included in bank position above)`]).number
  wsPL.mergeCells(`A${itHdrRn}:AD${itHdrRn}`)
  styleCell(wsPL.getCell(`A${itHdrRn}`), { size:9, fill:C.lightBlue })
  addPLRow([null, `Income tax paid — ${fy} Actual (for reference)`, ...zeros28()])
  ...
  const howToSteps = [
    ...
    ['Review the Assumptions sheet', 'Go to the Assumptions sheet. Every P&L account has a method (pct/fixed/manual) and value. Adjust as needed.'],
    ['Understand the Methods', '"pct" — enter a percentage to increase or decrease prior year  |  "fixed" — enter a total for the year  |  "manual" — same amount each month'],
